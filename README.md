# Windows path 环境变量清理/整理工具（Path-Cleanup）
...
- 脚本运行需要powershell 5.1 Windows7需手动安装后才可使用， Windows 10以上内置

## 使用方式

请先使用git克隆此仓库到本地电脑或者下载压缩包解压到本地目录。
- 运行前先右键ps1和bat脚本打开属性取消类似于自动阻止的勾选并点击应用，因为Windows默认会阻止在网络上下载的脚本运行。
- 虽然说脚本运行时也会自动解除这个标记但是也有概率无法自动解除,最好还是手动解除一下

如果`.ps1`已经关联了power shell推荐直接双击ps1脚本运行：
...
- 日常清理工具，请勿在系统损坏的状态下使用，否则可能造成系统进一步损伤
